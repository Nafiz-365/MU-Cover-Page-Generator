...
module.exports = async (req, res) => {
  ...
  try {
    ...
    console.log(`Evaluated in ${Date.now() - startTime}ms. Generating Image...`);
    ...
    console.log(`Image generated in ${Date.now() - startTime}ms. Total size: ${imageBuffer.length} bytes`);
    ...
    res.status(500).json({ error: 'IMAGE_GENERATION_FAILED', message: err.message, stack: err.stack });
  } finally {
    if (browser) {
      await browser.close();
    }
